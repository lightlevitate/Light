...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {// Smooth scrolling for navigation links
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
        e.preventDefault();
        const target = document.querySelector(this.getAttribute('href'));
...
document.querySelectorAll('.expertise-card, .achievement-card, .timeline-item').forEach(el => {
    el.style.opacity = '0';
...
document.querySelectorAll('.expertise-card, .achievement-card').forEach(card => {
...
console.log(`
🔒 Light Walker - Cyber Security Specialist
🐛 Bug Bounty Hunter & Penetration Tester
🌐 Website: https://lightwalker.security
📧 Email: light.walker@security.com
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Disable image saving by replacing src with data URI
    document.querySelectorAll('img').forEach(function(img) {
        const originalSrc = img.src;
        img.addEventListener('load', function() {
            // Convert to canvas to make it harder to save
            ...
            ctx.font = '20px Arial';
            ctx.fillText('Light Walker', 10, 30);
        });
    });
});
